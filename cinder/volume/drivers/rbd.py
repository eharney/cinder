...
class RBDDriver(driver.CloneableImageVD, driver.MigrateVD,
    ...
    def _do_flatten(self, volume_name: str, pool: str) -> None:
        ...
        try:
            with RBDVolumeProxy(self, volume_name, pool=pool) as vol:
                try:
                    vol.flatten()
                LOG.debug('flattening of %s has completed', image)
    ...
    def _flatten_children(self,
        ...
        rbd_image = RBDVolumeProxy(self, volume_name, ioctx=client_ioctx)
        try:
    ...
    def _delete_volume(self, volume: Volume, client) -> None:
        try:
            rbd_image = RBDVolumeProxy(self, volume.name, ioctx=client.ioctx)
        except self.rbd.ImageNotFound:
